...
description: "Use when generating a single HTML page for VotaFoundry / SemFoundry「个性交付」delivery — data dashboards or generic pages via PanelX-SDK real data, or custom shells/menus/entries via CDP-SDK (主题订阅、动作注册). Triggers: HTML 单页、数据看板、通用页、自定义外壳、内部资源、PanelX-SDK、CDP-SDK、custom-page-resource。"
---
...
- [内部资源工作流](references/internal-resources.md) — 页面 = 批次、list/etag 复用/顺序上传/写盘校验、错误语义、accessPath 红线。
- [系统资源注册表](references/system-resources.json) — 系统资源单一来源（`data-cdp-resource` 名称 + 精确 legacy URL）。
