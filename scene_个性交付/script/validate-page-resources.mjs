...
// ---- 系统资源注册表（单一来源） ----
// 技能仓库布局把该文件放在 references/，Scene 工作区把它镜像到 knowledge/；同名文件二选一。
const registryUrl = ['references', 'knowledge']
  .map((dir) => new URL(`../${dir}/system-resources.json`, import.meta.url))
  .find((url) => existsSync(url));
if (!registryUrl) throw new Error('未找到 system-resources.json（references/ 或其镜像目录）');
