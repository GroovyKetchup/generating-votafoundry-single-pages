...
| 位置 | 内容 | 用途 |
| --- | --- | --- |
| `/data/scene/d00433d5-e81b-4002-827f-c24c4a4604a5/knowledge/PanelX_SDK_文档_20251204_093233.md` | PanelX SDK 全量手册，涵盖 SDK 引入、环境探测、用户认证、数据查询、权限矩阵、按钮/事件调用、文件上传、存储模块等 API | 所有代码必须遵循此文档：通过 `https://kwaidoo.com/cdn_cdp/sdk/cdp_sdk/panelx-sdk-proxy.js` 引入并正确初始化 `PanelXSdkProxy`，调用真实接口、处理异常/权限。 |
...
3. **根据需求选择 Prompt 模板学习**：
   - **看板型** → `Prompt/databoard_generator_with_sdk_template.md`：包含 Tailwind + ECharts + Lucide 的标准骨架、关联字段配置 (`RELATION_FIELD_CONFIG`)、禁止 mock、Loading/错误反馈与刷新行为等红线；其中 Lucide 的“宿主共享优先、CDN 兜底”规则是强制约束，不得简化为直接 CDN 引入。
...
4. **设计与视效**：
   ...
   - 默认输出与 CDP 主题一致的风格，使用 `Tailwind CDN + Lucide + (ECharts 看板场景)`；其中 Lucide 必须优先复用 `window.semApp?.ui?.lucide`，仅当该实例不可用时才允许动态加载指定 CDN `https://kwaidoo.com/cdn_general/libs/lucide/0.562.0/umd/lucide.min.js` 兜底，禁止把 Lucide CDN 作为默认直引方案。若需求强调视觉表达，按该文档中的排版、配色、动效和构图原则执行。
...
5. **分支执行要点**：
   - **数据页面（看板/通用）**：
     ...
     - PanelX SDK 引入：`<script src="https://kwaidoo.com/cdn_cdp/sdk/cdp_sdk/panelx-sdk-proxy.js"></script>`。
...
6. **交付前自检**：
   ...
   - **Lucide 图标资源项（如使用图标则必须检查）**
     ...
     - 仅当 `window.semApp?.ui?.lucide` 不存在时，才允许动态创建 `<script>` 加载指定 Lucide CDN：`https://kwaidoo.com/cdn_general/libs/lucide/0.562.0/umd/lucide.min.js`
     - 禁止在 `<head>` 或 HTML 顶层无条件直引 `lucide.min.js`
     - 禁止使用 `@latest`、`unpkg.com` 或其他非指定地址作为交付版本
     - 禁止为了“自包含页面”“快速交付”“减少代码”跳过宿主资源判断
     - 静态 `data-lucide` 渲染后必须调用 `createIcons()`；动态插入、替换或切换图标后必须再次渲染
   - **数据页面**
     - 已使用真实接口且无 mock 常量
     - 统一通过 `https://kwaidoo.com/cdn_cdp/sdk/cdp_sdk/panelx-sdk-proxy.js` 引入并正确初始化 SDK
     - 关联字段通过硬编码配置映射
